**Note:** This document is a copy from the
[original markdown spec](https://daringfireball.net/projects/markdown/syntax.text)
by John Gruber just to test out markdown rendering on this site.

It remains copyright © 2002–2023 The Daring Fireball Company LLC.
...
[see the source for it by adding '.text' to the URL][src].
...
[Setext][1], [atx][2], [Textile][3], [reStructuredText][4], [Grutatext][5], and
[EtText][6] -- the single biggest source of inspiration for Markdown's syntax is
...
_writing_ for the web.

Markdown is not a replacement for HTML, or even close to it. Its syntax is very
small, corresponding only to a very small subset of HTML tags. The idea is _not_
...
read, write, and edit prose. HTML is a _publishing_ format; Markdown is a
_writing_ format. Thus, Markdown's formatting syntax only addresses issues that
...
Unlike block-level HTML tags, Markdown syntax _is_ processed within span-level
tags.
...
Similarly, because Markdown supports [inline HTML](#html), if you use angle
brackets as delimiters for HTML tags, Markdown will treat them as such. But if
you write:
...
are _always_ encoded automatically. This makes it easy to use Markdown to write
...
In other words, a _number-period-space_ sequence at the beginning of a line. To
...
-- but they are _not_ case sensitive. E.g. these two links:
...
The _implicit link name_ shortcut allows you to omit the name of the link, in
...
allowing for two styles: _inline_ and _reference_.
